...
async function syncGuideToKB() {
    ...
    try {
        ...
        try {
            ...
            // We need to join with org_members to get org_id and admin status
            const userRes = await client.query(`
                SELECT u.id, om.org_id 
                FROM users u
                JOIN org_members om ON u.id = om.user_id
                WHERE (om.is_admin = true OR u.is_superadmin = true) 
                AND om.is_active = true
                LIMIT 1
            `);

            if (userRes.rows.length === 0) {
                console.error('No admin user found to assign KB documents to.');
                return;
            }

            const admin = userRes.rows[0];
            console.log(`Using admin user: ${admin.id} (Org: ${admin.org_id})`);

            // Format content into a single markdown string
            let fullContent = "# Operon User Guide\n\n";

            Object.values(guideData.sections).forEach(sectionArray => {
                sectionArray.forEach(section => {
                    fullContent += `## ${section.title}\n\n`;
                    fullContent += `${section.description}\n\n`;
            ...
            const title = "Official Operon User Guide";
            const category = "System Documentation";
            const tags = JSON.stringify(["user-guide", "help", "system"]);
            ...
            if (existingRes.rows.length > 0) {
                // Update existing
                console.log('Updating existing User Guide document...');
                await client.query(`
                    UPDATE knowledge_base 
                    SET content = $1, updated_at = NOW()
                    WHERE id = $2
                `, [fullContent, existingRes.rows[0].id]);
                console.log('User Guide updated successfully.');
            } else {
                // Insert new
                console.log('Creating new User Guide document...');
                await client.query(`
                    INSERT INTO knowledge_base (org_id, title, content, category, tags, created_by)
                    VALUES ($1, $2, $3, $4, $5, $6)
                `, [admin.org_id, title, fullContent, category, tags, admin.id]);
