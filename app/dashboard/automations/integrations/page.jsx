...
function IntegrationsContent() {
    ...
    const fetchIntegrations = async () => {
        try {
            ...
            if (!token || token === 'null' || token === 'undefined') {
                setErrorMessage('Please sign in to view integrations.');
                setLoading(false);
                return;
            }
    ...
    const handleConnectClick = async (integration, method = 'credentials') => {
        if (method === 'oauth2') {
            ...
            try {
                const token = localStorage.getItem('auth_token');
                if (!token || token === 'null' || token === 'undefined') {
                    setErrorMessage('Please sign in to connect integrations.');
                    return;
                }
    ...
    const handleDisconnect = async (integration) => {
        ...
        try {
            const token = localStorage.getItem('auth_token');
            if (!token || token === 'null' || token === 'undefined') {
                setErrorMessage('Please sign in to manage integrations.');
                return;
            }
